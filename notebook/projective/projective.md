...
在3D空间中absolute conic的方程式已知的，其二阶曲线矩阵C=I，见"absolute conic"章节。由于absolute conic是在无穷远处，所以可以直接使用上面的公式，得到absolute conic在相机中所成的像，该像只与相机内参和转向角度有关。absolute conic在图像中所成像的二阶曲线矩阵$\omega = (KR)^{-T}I(KR)^{-1}=(KK^T)^{-1}$.
